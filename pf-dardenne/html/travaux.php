 <!DOCTYPE html>
 <html lang="fr">

 <head>
     <meta charset="UTF-8">
     <meta http-equiv="X-UA-Compatible" content="IE=edge">
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <title>PF - Dardenne | Mes travaux</title>
     <link rel="stylesheet" href="../css/style.css">
     <link rel="stylesheet" href="../css/reset.css">
     <link rel="stylesheet" href="../css/travaux.css">
 </head>

 <body>
     <header>
         <?php include 'header.php' ?>
     </header>
     <!-- Debut MAIN -->


     <main>
         <h2 class="titre_card">MES TRAVAUX</h2>

         <section class='container_card_travaux'>

             <a href="travaux_creation.php" class="marge_card">
                 <div class="card_travaux un">
                     <div class='titre_card_travaux'>
                         <h3>CRÉATION TATOOM</h3>
                     </div>
                 </div>
             </a>

             <a href="travaux_photo.php" class="marge_card">
                 <div class="card_travaux deux">
                     <div class='titre_card_travaux'>
                         <h3>PHOTOGRAPHIE</h3>
                     </div>
                 </div>
             </a>

             <a href="travaux_web.php" class="marge_card">
                 <div class="card_travaux trois">
                     <div class='titre_card_travaux'>
                         <h3>SITES WEB</h3>
                     </div>
                 </div>
             </a>

             <a href="travaux_projet.php" class="marge_card">
                 <div class="card_travaux quatre">
                     <div class='titre_card_travaux'>
                         <h3>PROJET PERSO</h3>
                     </div>
                 </div>
             </a>

             <a href="travaux_veille.php" class="marge_card">
                 <div class="card_travaux cinq">
                     <div class='titre_card_travaux'>
                         <h3>VEILLE</h3>
                     </div>
                 </div>
             </a>

             <a href="travaux_pro.php" class="marge_card">
                 <div class="card_travaux six">
                     <div class='titre_card_travaux'>
                         <h3>TRAVAUX PRO</h3>
                     </div>
                 </div>
             </a>

         </section>


     </main>
     <!-- FIN MAIN -->

     <footer>
         <?php include 'footer.php' ?>
     </footer>
 </body>
 <script src="../Js/main.js"></script>

 </html>